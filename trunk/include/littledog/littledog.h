...
//
#define LITTLEDOG_SOFTWARE_VERSION_STRING "1.0.4"
#define LITTLEDOG_SOFTWARE_VERSION_MAJOR 1
#define LITTLEDOG_SOFTWARE_VERSION_MINOR 0
#define LITTLEDOG_SOFTWARE_VERSION_POINT 4

//
//  For software compatibility checks between Linux and QNX
//
#define LITTLEDOG_PROTOCOL_VERSION_NUMBER "1.0.1"
...
enum LD_ERROR_CODE
{
	...
	LD_BAD_SEGMENT_INDEX,                  //!<  Invalid segment index
...
//!          - if the robot has already been calibrated it will return immediately
...
//!      - user calls runTrial()
//!          - LittleDog calls virtual initControl() once
...
class LittleDog
{
 ...
 public:
	...
	enum State
	{
		...
		STATE_TRIAL_RUNNING,      //!<  Trial is running
	...
	//!  \enum
	//!
	//!  \brief   Leg segment indices.
	//!
	enum LegSegment
	{
		UPPER_LEG = 0,      //!<  Upper leg
		LOWER_LEG,          //!<  Lower leg
		NUM_LEG_SEGMENTS    //!<  Number of leg segments
	};
	...
	{ 
		...
		float      force;                 //!<  Measured force at the foot
	...
	struct IMUInfo
	{
		bduVec3f orientation;   //!<  Body orientation as estimated by the IMU in yaw-pitch-roll Euler angles
	...
	//!    includes 'torque saturation' and 'at limit' flags as
	...
	{
		LEG_MODE_INDEPENDENT,  //!< The joints are controlled independantly
	...
	enum MocapConstants
	{
		MAX_MARKERS  = 45,        //!<  Maximum number of markers supported by the mocap system
		MAX_TERRAINS =  3,        //!<  Maximum number of terrain boards in a trial
		MAX_TERRAIN_MARKERS = 5   //!<  Maximum number of markers on a terrain board
	};
	...
	struct MocapBodyInfo
	{
		...
		bduVec3f   m_orientation;          //!<  3d vector of orientation, in yaw-pitch-roll Euler angles
	...
	enum TerrainID
	{
		...
		NUM_TERRAINS                  //!<  Total number of possible terrain boards
	};
	...
	struct TerrainInfo
	{
		...
		bduVec3f       m_orientation;  //!<  3d position in global coordinates, in Euler angles
		int            m_age;          //!<  number of frames this data is out of date
	};
 ...
 public:
	...
	//!    used when creating LittleDog data set files.
	//!
	...
	//!    will be saved before this call returns.
	//!
	//!  \_State_Information
	//!
	...
	//!    STATE_CALIBRATED.
	//!
	//!    While the trial is running the system state will be
	//!    STATE_TRIAL_RUNNING.
	//!
	...
	//!
	//!    This function is the preferred way to prematurely stop
	...
	//!    STATE_ROBOT_CALIBRATING or STATE_TRIAL_RUNNING.  It
	...
	//!  \_Description
	//!
	//!    This funtion returns the computed average packet loss of robot-source
	...
	//!    \_out  vel    - current joint velocity
	//!    \_out  tau_d  - current joint torque command
	//!
	//!    If output variables are NULL pointers the information will
	//!    not be returned.
	//!
	//!  \_Returns         LD_ERROR_CODE result
	//!
	...
	//!
	//!    \_in   leg    - leg index
	//!    \_out  force  - returned force
	//!
	//!  \_Returns         LD_ERROR_CODE result
	//!
	LD_ERROR_CODE getFootForce( LegIndex leg, float * force ) const;
	...
	//!    \_out  id      - terrain identifier ( TERRAIN_NONE - TERRAIN_Z )
 ...
 protected:
	...
	//!    trial while the system is in STATE_TRIAL_RUNNING.
	//!
	//!    This function should not be called directly by the user.
	//!
	...
	//!  \_Description
	//!
	//!    This virtual function is called at the end of a
	//!    trial when the system is moving from STATE_TRIAL_RUNNING
	//!    to STATE_ROBOT_CALIBRATED.
	//!
	//!    This function should not be called directly by the user.
	//!
	//!    The default implementation does nothing; any necessary
	...
	//!
	//!    Unless otherwise specified, these functions should be called
	//!    only when the system is in STATE_TRIAL_RUNNING.
	//!
	//! @{
	//------------------------------------------------------------------
